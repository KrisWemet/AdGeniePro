...
def test_clickbank_uses_external_click_id_parameter():
    """ClickBank S2S can return extclid, so this is the attribution join key."""
    ...
    assert param == "extclid"
    assert "extclid=click-123" in url
    assert "subid=" not in url


def test_clickbank_network_name_is_case_insensitive():
    assert _network_click_id_param("ClickBank") == "extclid"
